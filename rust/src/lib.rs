...
mod tests {
    ...
    #[test]
    fn test_safe_cap_has_rights() {
        let cap = SafeCap { raw: 0b1010 };
        assert!(cap.has_rights(0b1010));
        assert!(cap.has_rights(0b1000));
        assert!(!cap.has_rights(0b0100));
        assert!(!cap.has_rights(0b1111));
    }

    #[test]
    fn test_rust_cap_has_rights() {
        let cap = SafeCapability { raw: 0b0111 };
        assert!(rust_cap_has_rights(&cap as *const _, 0b0011));
        assert!(!rust_cap_has_rights(&cap as *const _, 0b1000));
        assert!(!rust_cap_has_rights(core::ptr::null(), 0b0001));
    }

    #[test]
    fn test_handle_command_security() {
        let help = b"help";
        assert_eq!(rust_handle_command(help.as_ptr(), help.len()), 42);
        let dangerous = b"rm -rf /; cat /etc/passwd";
        assert_eq!(rust_handle_command(dangerous.as_ptr(), dangerous.len()), -1);
    }

    #[test]
    fn test_demand_page_fault_isolation() {
        assert_eq!(rust_handle_demand_page_fault(0x7FB500, 0, false, 1), DemandAction::Invalid);
        assert_eq!(rust_handle_demand_page_fault(0xFFC00000, 0, false, 1), DemandAction::Invalid);
        assert_eq!(rust_handle_demand_page_fault(0x500000, 4, false, 1), DemandAction::DemandZero);
        assert_eq!(rust_handle_demand_page_fault(0x450000, 2 | 4, true, 2), DemandAction::CowCopyNeeded);
        assert_eq!(rust_handle_demand_page_fault(0x450000, 2 | 4, true, 1), DemandAction::NoAction);
    }

    #[test]
    fn test_validate_fs_operation_policy() {
        assert_eq!(rust_validate_fs_operation(1, FsOp::Lookup as u32, 0b0001, b"safe.txt".as_ptr(), 8), 0);
        assert_eq!(rust_validate_fs_operation(1, FsOp::Create as u32, 0, b"newfile".as_ptr(), 7), -1);
    }
}
