...
/*
 * Horus Real Userspace Filesystem Server (feature-rich v1)
 *
 * This runs as a normal userspace task. It owns a complete in-memory
 * hierarchical filesystem with directories, files, basic encryption,
 * and serves clients over synchronous IPC.
 *
 * Protocol is simple request/reply using the existing IPC primitives.
 * The server creates its own listening endpoint on startup.
 *
 * Feature set:
 *   - Hierarchical directories with multiple levels
 *   - Files with data, size, owner
 *   - Transparent (demo) encryption on some files
 *   - Operations: lookup, create, mkdir, delete, readdir, read, write, stat
 *   - Rich demo tree created automatically
 *   - All operations go through capability-style handles (server side)
 */
...
struct fs_request {
    ...
    uint32_t handle;
    char name[32];
    uint32_t type;     /* 1=file 2=dir for create/mkdir */
    uint32_t rights;
    uint32_t offset;
    uint32_t len;
    uint8_t  data[128];
...
/* (find_child kept for future IPC client use) */
...
int main(void) {
    ...
    /* Register ourselves as the system FS server using our pre-created endpoint slot 4 */
    ...
    for (;;) {
        ...
        case OP_READ: {
            if (target && target->type == 1) {
                int nbytes = req.len > (int)target->size ? target->size : req.len;
        ...
        case OP_WRITE: {
            if (target && target->type == 1) {
                ...
                if (w > 0) {
                    ...
                    if (req.offset + w > (int)target->size) target->size = req.offset + w;
                }
                rep.rc = w;
            } else {
                rep.rc = -1;
