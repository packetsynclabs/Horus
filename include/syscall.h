...
#define AUDIT_IPC           6
#define AUDIT_FS            7   /* Capability FS operations */

/* Finer-grained audit event types (must match kernel) */
...
/* Note: CAP_USER is a kernel capability type (slot 6 by convention for admin tasks) */
...
/* Extended syscall for FS ops needing >3 args (loads esi/edi from frame) */
...
/* First-class secure program loading (receive over loader TCP transport + spawn) */
...
/* sudo: authenticate as current user with pass, then spawn armed image as root */
static inline int sys_sudo(const char *pass) {
    return syscall(SYS_SUDO, (uint32_t)pass, 0, 0);
}

/* Secure password read - no echo, returns length */
...
/* FS lookup: dir_cap_slot, name, out_slot, desired_rights */
...
/* FS create: dir_cap_slot, name, type, out_slot, desired_rights */
...
static inline int sys_block_read(uint64_t block, void *buf, uint32_t len) {
    /* Use the 6-arg extended syscall (block high/low + buf + len) */
